# IMager bot
## О чем речь?
IMager - пакет, позволяющий собрать картинку из картинок, тематично скачанных с сайта [Fonwall](https://fonwall.ru/), или собственноручно загруженных.
...
## Дополнительная информация об исполнении
### Наложение оригинала
...
3.1. Для начала [скачаем](#команда-download_topic) тестовые набор картинок:

```python manage.py download_topic```<br>
Возможно так (далее вариативность команд будет опускаться):<br>
```poetry run python3 manage.py download_topic```

Введем тему скачиваемых картинок, например - `Коты`<br>
Далее выбираем создать новую таблицу - `0` <br>

Начинается парсинг `Парсим...`
Через примерно 5 секунд парсинга жмем комбинацию (Ctrl + C), потому что в зависимости от темы спарсить можно от 2 до 3 000 картинок.<br>
После начнется более долгое скачивание, которое также можно прервать.

После этого начнется запись в бд. Если случайно прервали запись в БД, то поможет [команда обновления БД](#команда-update_dp)

3.2 Посмотрим [распределение картинок на палитре](#команда-show_db_distribution)

```python3 manage.py show_db_distribution```

Выбираем единственную таблицу - `0`<br>

3.3 [Протестируем механику](#команда-test_engine)

```python3 manage.py test_engine```

Выбираем единственную таблицу - `0`<br>

Вводим абсолютный муть до желаемой картинки (*.jpg, *.jpeg) - `<*.jpg>`

На выводе получаем путь к результату. Эта единственная картинка находится по пути `*/IMager/content/results/<orig_name>res.png`

4. Подготовка бота.

4.1. Токен
Для работы бота нужен ТОКЕН. Переменную токена следует создать в файле `*/IMager/.env`<br>
В целом о ботах можно почитать [здесь](https://tlgrm.ru/docs/bots), а для дальнейшей работы может понадобиться [эта документация](https://core.telegram.org/bots/api)

```TOKEN='<TOKEN>'```

4.2. Настройка тем для заполнения

Если все делали по примеру `Коты`, то ничего менять не надо. [Запускайте бота](#команда-start).

Иначе смотрите след пункт

```python manage.py start```

4.3. Настройка интерфейса

FSM бота собирвет информацию по принципу ключ-значение, где ключи это кнопки интерфейса пользователя, а значения определяются в `*\IMager_bot\settings\config.py`, а именно после комментария `# TG KEYBOARDS`

4.3.1. topics

topics = {'<Название кнопки>': <таблица тематики>}

*Какой бы не была тема при скачивании, она мохранится транслитом в lowercase*

Просмотр таблиц темати эквивалентен названиям директорий в транслите. Либо можно посмотреть при вызове команды [обновления таблицы](#команда-update_dp_single), которая выведет все директории, которые можно загрузить в БД.

4.3.2. noise_degrees

noise_degrees - [степень шума](#1-степень-шума)

noise_degrees = {'<Название кнопки>': (rand1, rand2)}

4.3.3. new_images_sizes

new_images_sizes - [размер нового изображения](#2-размер-изображения)

new_images_sizes  = {'<Название кнопки>': <количество картинок по большей стороне>}

*Этого достаточно для старта бота. Чтобы узнать о нем больше - свяжитесь с разработчиком.*
...
В данной версии программы используется 2 вариант из-за особенности алгоритма, ускоряющего работу.
...
Команда позволяет скачать картинки с сайта [Fonwall](https://fonwall.ru/).
